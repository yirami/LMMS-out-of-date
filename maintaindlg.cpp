...
maintainDlg::maintainDlg(QWidget *parent)
    : QDialog(parent)
{
    ...
    this->setWindowTitle(QString("数据库维护"));
    // 数据部分
    // 主数据库
    QSqlDatabase mainDB = QSqlDatabase::database(mainName);
    // 功能临时数据库

    QSqlTableModel *maintainModel = new QSqlTableModel(this,mainDB);
    maintainModel->setTable("Medicine");
    maintainModel->select();//选取表的所有行
    tabV->setModel(maintainModel);
    tabV->show();
}
maintainDlg::~maintainDlg()
{
